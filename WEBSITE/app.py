...
def market_demo_tick():
    ...
    for s in stocks:
        ...
        cur = s.current_price if s.current_price is not None else s.base_price or 0.0
        new_price = max(0.01, round(cur * (1 + pct_change), 2))
        s.day_high = max(s.day_high or new_price, new_price)
        s.day_low = min(s.day_low or new_price, new_price)
        s.current_price = new_price
        s.currentMarketPrice = new_price
    db.session.commit()
    ...
    return jsonify([{
        ...
        "low_price": s.day_low,
        "quantity": s.quantity,
        "market_cap": round((s.current_price or 0.0) * (s.quantity or 0), 2)
    } for s in stocks])
...
def market():
    ...
    for s in stocks:
        ...
        open_price = today_summary.open_price if today_summary else (s.base_price or 0.0)
        high_price = today_summary.high_price if today_summary else (s.current_price or s.base_price or 0.0)
        low_price = today_summary.low_price if today_summary else (s.current_price or s.base_price or 0.0)
...
def trade(ticker):
    ...
    if request.method == 'POST':
        ...
        total = round((stock.current_price or 0.0) * quantity, 2)
        if quantity <= 0:
        ...
        if action == "BUY":
            ...
            if not p or p.quantity < quantity:
                ...
                proceeds = round((stock.current_price or 0.0) * quantity, 2)
                user.funds += proceeds
                stock.quantity += quantity
                p.quantity -= quantity
                if p.quantity == 0:
...
def execute_order(ticker):
    ...
    order = Order(user_id=user.id, stock_id=stock.stockId, action=action.upper(), quantity=quantity, price_per_stock=stock.current_price, total_amount=total, status='executed', executed_at=datetime.utcnow())
    db.session.add(order)
    db.session.commit()
...
def simulate_fast_ticks():
    ...
    for stock in stocks:
        ...
        base = stock.base_price or 0.0
        stock.current_price = round(base + change, 2)
        ...
        stock.currentMarketPrice = stock.current_price
    db.session.commit()
    return redirect(url_for("admin_console"))
...
def price_update(ticker):
    ...
    return jsonify({'ticker': s.ticker, 'current_price': s.current_price, 'data': data})
